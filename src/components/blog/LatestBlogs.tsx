...
export default function LatestBlogs({ posts }: LatestBlogsProps) {
  const visible = (posts || []).filter((p) => p.visibility !== false);
      ...
      >
        {visible.map((post) => (
          ...
          >
            ...
            <Link
              href={`/blog/${post.slug}`}
              className="aspect-[4/3] rounded-[var(--radius-md)] bg-gradient-to-br from-[var(--color-neutral-900)] to-[var(--color-neutral-950)] text-[var(--color-neutral-0)] p-6 relative overflow-hidden flex flex-col justify-end shadow-sm group-hover:shadow-md transition-all duration-300"
            >
              <div className="relative z-10">
                <span className="text-xl md:text-2xl font-bold leading-tight drop-shadow-sm text-balance block group-hover:text-[var(--color-accent-primary)] transition-colors">
                  {post.title}
                </span>
              </div>
            </Link>
            ...
            <div className="mt-4 flex flex-col flex-1">
              <h3 className="text-[length:var(--text-heading-h4)] font-[number:var(--font-weight-semibold)] text-[var(--color-text-primary)] transition-colors line-clamp-2 leading-snug">
                <Link href={`/blog/${post.slug}`} className="hover:text-[var(--color-accent-primary)] transition-colors">
                  {post.title}
                </Link>
              </h3>
